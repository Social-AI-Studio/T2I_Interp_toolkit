...
class PAnnealTrainer(SAETrainer):
    ...
    def __init__(
        ...
        decay_start: int | None = None,  # step at which to start decaying lr
        sparsity_warmup_steps: int | None = 2000,  # number of steps to warm up sparsity penalty
        ...
        anneal_end: int | None = None,  # step at which to stop annealing, defaults to steps-1
        ...
        n_sparsity_updates: int
        | str = 10,  # number of times to update the sparsity penalty, at most steps-anneal_start times
        sparsity_queue_length: int = 10,  # number of recent sparsity loss terms, onle needed for adaptive_sparsity_penalty
        resample_steps: int | None = None,  # number of steps after which to resample dead neurons
        ...
        if step in self.sparsity_update_steps:
            ...
            if step >= self.sparsity_update_steps[self.p_step_count]:
                ...
                if self.next_p is not None:
                    ...
                    self.sparsity_coeff = (
                        self.sparsity_coeff * (local_sparsity_new / local_sparsity_old).item()
                    )
                # Update p
