...
def _resolve_includes(node: Any, base_dir: pathlib.Path,
                      seen: set[Tuple[pathlib.Path, str]] | None = None) -> Any:
...
def build_module_mapper(hf_model: Any, top_yaml_path: str, *, strict: bool = False) -> Dict[str, str]:
    """
                ...
                if isinstance(spec, dict):
                    ...
                    for sublabel, subpath in inner.items():
                        if not isinstance(subpath, str):
                            if strict:
                                raise TypeError(f"[{ctx}] child slot '{sublabel}' must be str, got {type(subpath)}")
                            continue
