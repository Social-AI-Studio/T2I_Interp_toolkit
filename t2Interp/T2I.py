...
CONFIG_MAP = [
    (_by_type(CLIPTextModel), "./config/CLIPEncoder.yaml"),
...
REGISTRY.extend(
    [
        (_by_type(CLIPTextModel), _from_comp_key("clip_encoder"), ClipEncoder),
...
class T2IModel(DiffusionModel):
    ...
    def run_with_cache(
        self, prompt, accessors: list[ModuleAccessor], **kwargs
    ) -> list[ModuleAccessor, torch.Tensor]:
        ...
        with self.generate(
            prompt, num_inference_steps=num_inference_steps, seed=seed, **generate_kwargs
        ) as tracer:
            dtype, device, detach = (
