...
class FluxTransformer:
    ...
    def __init__(self, transformer: th.nn.Module):
        if not hasattr(transformer, "dual_stream_blocks") or not hasattr(
            transformer, "single_stream_blocks"
        ):
        ...
        self.dual_stream_blocks: dict[int, DualStreamBlock] = {
            i: DualStreamBlock(
                layer=i,
                in_=ModuleAccessor(dual_stream_blocks[i], "clip_encoder_input", IOType.INPUT),
                out_=ModuleAccessor(dual_stream_blocks[i], "clip_encoder_output", IOType.OUTPUT),
                attn_in=ModuleAccessor(
                    dual_stream_blocks[i].self_attn, "clip_encoder_attn", IOType.INPUT
                ),
                attn_out=ModuleAccessor(
                    dual_stream_blocks[i].self_attn, "clip_encoder_attn", IOType.OUTPUT
                ),
                mlp_in=ModuleAccessor(
                    dual_stream_blocks[i].mlp, "clip_encoder_mlp_in", IOType.INPUT
                ),
                mlp_out=ModuleAccessor(
                    dual_stream_blocks[i].mlp, "clip_encoder_mlp_out", IOType.OUTPUT
                ),
            )
        ...
        self.single_stream_blocks: dict[int, SingleStreamBlock] = {
            i: SingleStreamBlock(
                layer=i,
                in_=ModuleAccessor(single_stream_blocks[i], "clip_encoder_input", IOType.INPUT),
                out_=ModuleAccessor(single_stream_blocks[i], "clip_encoder_output", IOType.OUTPUT),
                attn_in=ModuleAccessor(
                    single_stream_blocks[i].self_attn, "clip_encoder_attn", IOType.INPUT
                ),
                attn_out=ModuleAccessor(
                    single_stream_blocks[i].self_attn, "clip_encoder_attn", IOType.OUTPUT
                ),
                mlp_in=ModuleAccessor(
                    single_stream_blocks[i].mlp, "clip_encoder_mlp_in", IOType.INPUT
                ),
                mlp_out=ModuleAccessor(
                    single_stream_blocks[i].mlp, "clip_encoder_mlp_out", IOType.OUTPUT
                ),
            )
