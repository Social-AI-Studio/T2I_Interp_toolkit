...
class CLIPScorer:
    ...
    def compute(self, out: Output, prompts: list[str] = None) -> Output:
        ...
        if prompts is None:
            if hasattr(out, "labels") and out.labels is not None:
                ...
                hasattr(out, "run_metadata")
                and out.run_metadata is not None
                and "prompts" in out.run_metadata
            ):
                prompts = out.run_metadata["prompts"]
            else:
                raise ValueError(
                    "prompts must be provided either as argument or in out.labels / out.run_metadata['prompts']"
                )
        ...
        if len(prompts) != len(images):
            ...
            if len(prompts) == 1:
                ...
                raise ValueError(
                    f"Number of prompts ({len(prompts)}) does not match number of images ({len(images)})"
                )
    ...
    @torch.no_grad()
    def score(
        self, images: str | Image.Image | list, prompts: str | list, **kwargs
    ) -> dict[str, list[float]]:
        """
