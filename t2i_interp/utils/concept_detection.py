...
class ConceptLocalizer:
    ...
    def get_mask(
        self, image: Image.Image, text: str, threshold: float = 0.5, dominant_region: bool = False
    ) -> torch.Tensor:
        if image.mode != "RGB":
    ...
    def get_topk_heads(
        ...
        concept_hs = (
            hs[:, concept_indices, ...] if orig_ndim == 3 else hs[concept_indices, ...]
        )  # [B, N_selected, H, d] or [N_selected, H, d
        concept_hs = concept_hs.sum(dim=-1)  # [B, N_selected, H] or [N_selected, H]
        if orig_ndim == 3:
            concept_hs = concept_hs.sum(dim=0)  # [N_selected, H]
        # get top-k heads
        _, topk_heads = torch.topk(
            concept_hs, k=topk, dim=-1, largest=True, sorted=True
        )  # [B, N_selected, topk] or [N_selected, topk]
        return topk_heads
