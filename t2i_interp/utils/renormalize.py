...
def renormalizer(source="zc", target="zc"):
    ...
    if isinstance(source, str):
        ...
        oldoffset, oldscale = (
            (normalizer.mean, normalizer.std) if normalizer is not None else OFFSET_SCALE["pt"]
        )
    newoffset, newscale = target if isinstance(target, tuple) else OFFSET_SCALE[target]
    return Renormalizer(oldoffset, oldscale, newoffset, newscale, tobyte=(target == "byte"))
...
class Renormalizer:
    def __init__(self, oldoffset, oldscale, newoffset, newscale, tobyte=False):
        self.mul = torch.from_numpy(numpy.array(oldscale) / numpy.array(newscale))
        self.add = torch.from_numpy(
            (numpy.array(oldoffset) - numpy.array(newoffset)) / numpy.array(newscale)
        )
        self.tobyte = tobyte
