...
class BatchIterator(Iterator[list[T]]):
    ...
    def __init__(
        ...
        *,
        source_factory: Callable[[], Iterable[T] | Iterator[T]] | None = None,
    ):
        if batch_size <= 0:
...
def convert_buffer_to_memap(
    ...
    D: int | None = None,  # feature dimension; if None inferred from first batch
    ...
    while True:
        ...
        if not isinstance(batch, torch.Tensor):
            ...
            if isinstance(batch, dict):
                ...
                raise TypeError(
                    "Expected tensor or dict with a tensor value from buffer.__next__()."
                )
...
class CachedActivationIterator:
    ...
    def __init__(self, it: Iterable[torch.Tensor], **kwargs) -> None:
        ...
        self.gpu_device: str | None = kwargs.get(
            "gpu_device", None
        )  # e.g., "cuda" or None to keep on CPU
...
def cache_path(dataset: str, accessor_name: str, split: str, subset: str | int | None) -> str:
...
def normalize_batch(
    batch: torch.Tensor | list[Any] | tuple[Any, ...], device: str
) -> list[torch.Tensor]:
    """
    ...
    """
    # Normalize input container to list
    if is_tensor(batch):
        items = [batch]
    ...
    for item in items:
        if is_tensor(item):
            item = item.to(device)
            # Squeeze (N, 1) labels/outputs if necessary
            # Note: For classification labels (N, 1) -> (N,) is required.
            # For scalar outputs (N, 1) it might be intended, but CrossEntropyLoss requires (N, C).
            # So (N, 1) is usually a label or single-head output that needs squeezing?
            # Be careful with Binary Classification if (N, 1) output expected.
            # But user requested this squeeze logic specifically for labels.
            if item.ndim == 2 and item.shape[1] == 1:
                item = item.squeeze(1)
