    ...
    for img in schema.get("encoders", {}).get("image", []):
        ...
        if img.get("type") == "vae" and "vae" in img:
            ...
            _discover_and_add(
                ...
                names,
                f"{base}.vae.decoder",
                vae["decoder"]["discover"],
                vae["decoder"]["slots"],
            )
            if "misc" in vae:
                _discover_and_add(
                    out, names, f"{base}.vae.misc", vae["misc"]["discover"], vae["misc"]["slots"]
                )
