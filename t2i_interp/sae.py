...
class SAEManager:
    ...
    def _ensure_bank(self, edited_model: Any) -> nn.ModuleDict:
        ...
        if not hasattr(edited_model, "_sae_bank"):
            edited_model._sae_bank = nn.ModuleDict()
        bank = edited_model._sae_bank
    ...
    def _get_sae_dim(self, sae: nn.Module) -> int:
        ...
        raise ValueError(
            "Cannot determine input dimension of SAE. Ensure it has .encoder.weight or .config.d_in or .d_in"
        )

    def _flatten_for_sae(
        self, x: t.Tensor, sae: nn.Module
        ...
        if not dims:
            raise RuntimeError(
                f"SAE expects input dim {d_in}, but input shape is {x.shape}. No dimension matches."
            )
    ...
    def _encode_decode(
        ...
        sae: nn.Module,
        x_flat: t.Tensor,
        z_alter_fn: Callable[[t.Tensor], t.Tensor] | None = None,
    ...
    def _convert_list_to_specs(
        self,
        sae_list: list[SAEHookSpec] | list[tuple[ModuleAccessor, nn.Module, str]],
        use_delta: bool | dict[str, bool],
    ) -> list[SAEHookSpec]:
        specs = []
    ...
    def capture_identity(
        ...
        *,
        cache: dict[str, list[dict[str, t.Tensor]]] | None = None,
        pick_input: Callable[[tuple[Any, ...]], t.Tensor] | None = None,
    ):
        """
        ...
        def cm():
            ...
            with boundary as edited_model:
                ...
                edited_model._sae_cache = cache

                for spec in specs:
                    accessor, sae, name, use_delta = (
                        spec.accessor,
                        spec.sae,
                        spec.name,
                        spec.use_delta,
                    ...
                    # Identify IOType
                    io_type = getattr(
                        accessor, "io_type", IOType.OUTPUT
                    )  # Default to OUTPUT if missing
                    ...
                    if io_type == IOType.OUTPUT:
                        ...
                        def _pre_hook(module, inputs, _use_pick=pick_input):
                            ...
                            input_stash[module].append(
                                x_in.detach()
                            )  # Detach stash? Or keep grad? usually detached for delta calc if just for error?
                        ...
                        def _pre_hook_input(
                            module, inputs, _sae=sae, _name=name, _use_pick=pick_input
                        ):
                            # inputs is a tuple
                            x_in = _use_pick(inputs) if _use_pick is not None else inputs[0]
    ...
    def edit_with_encoder_alter(
        ...
        specs: list[SAEHookSpec],
        *,
        z_alter_fns: dict[str, Callable[[t.Tensor], t.Tensor]] | None = None,
        cache: dict[str, list[dict[str, t.Tensor]]] | None = None,
        pick_input: Callable[[tuple[Any, ...]], t.Tensor] | None = None,
    ):
        """
            ...
            with boundary as edited_model:
                ...
                for spec in specs:
                    ...
                    if needs_encdec and not (hasattr(sae, "encoder") and hasattr(sae, "decoder")):
                        raise ValueError(f"SAE '{name}' must expose .encoder and .decoder")
                    ...
                    if io_type == IOType.OUTPUT:
                        ...
                        def _pre_hook_input(
                            module, inputs, _sae=sae, _name=name, _zfn=z_fn, _use_pick=pick_input
                        ):
                            # inputs is tuple
                            x_in = _use_pick(inputs) if _use_pick is not None else inputs[0]
    ...
    def capture_activations(
        ...
        prompt: str | list[str],
        *,
        use_delta: bool | dict[str, bool] = False,
        pick_input: Callable[[tuple[Any, ...]], t.Tensor] | None = None,
        ...
        **kwargs,  # passed to pipeline/generation
        ...
        cache = {}
        with self.capture_identity(specs, cache=cache, pick_input=pick_input) as edited_model:
    ...
    def run_with_steering(
        ...
        **kwargs,
        ...
        with self.edit_with_encoder_alter(
            specs, z_alter_fns=z_alter_fns, cache=cache, pick_input=pick_input
        ) as edited_model:
            if hasattr(edited_model, "pipeline"):
                out = edited_model.pipeline(prompt, **kwargs)
            else:
