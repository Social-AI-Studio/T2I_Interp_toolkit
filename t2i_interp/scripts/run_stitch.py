...
def main(cfg: DictConfig) -> None:
    ...
    lora_b_cfg = (
        OmegaConf.to_container(cfg.get("lora_b", {}), resolve=True)
        if hasattr(cfg, "lora_b")
        else {}
    )
    unet_b_cfg = (
        OmegaConf.to_container(cfg.get("unet_b", {}), resolve=True)
        if hasattr(cfg, "unet_b")
        else {}
    )
    ...
    model_a = T2IModel(
        cfg.model_key, automodal=DiffusionPipeline, device=cfg.device, dtype=cfg.dtype
    )
    ...
    if cross_model:
        ...
        if lora_b_repo:
            ...
            model_b = T2IModel(
                model_key_b or cfg.model_key,
                automodal=DiffusionPipeline,
                device=cfg.device,
                dtype=cfg.dtype,
            )
            if lora_b_scheduler:
            ...
            print(
                f"Cross-model (LCM-LoRA): model_a={cfg.model_key}  "
            ...
            print(
                f"Cross-model (custom UNet): model_a={cfg.model_key}  "
            ...
            model_b = T2IModel(
                model_key_b, automodal=DiffusionPipeline, device=cfg.device, dtype=cfg.dtype
            )
    ...
    if needs_dataset:
        if collect_to_memory:
            ...
            acts_a_train = collect_latents_inmemory(
                accessors=[cfg.layer_a],
                dataset=ds_train,
                model=model_a,
                columns=[cfg.prompt_col_a],
                batch_size=cfg.batch_size,
                max_samples=max_tr,
                num_inference_steps=n_steps_a,
            ...
            acts_b_train = collect_latents_inmemory(
                accessors=[cfg.layer_b],
                dataset=ds_train,
                model=model_b or model_a,
                columns=[cfg.prompt_col_b],
                batch_size=cfg.batch_size,
                max_samples=max_tr,
                num_inference_steps=n_steps_b,
                guidance_scale=cfg.guidance_scale,
                capture_step_index=capture_step,
                conditional_only=cfg.conditional_only,
                device=cfg.device,
    ...
    if mode == "steer_contrast":
        ...
        steered_paths = [
            os.path.join(cfg.output_dir, f"steered_{i}.png") for i in range(len(apply_prompts))
        ]
        baseline_paths = (
            [os.path.join(cfg.output_dir, f"baseline_{i}.png") for i in range(len(apply_prompts))]
            if steer_baseline
            else []
        )
        meta = {
    ...
    # ── Mode: steer_transfer — skip training, load mapper + steer vector ──────
        ...
        if not mapper_path or not steer_vec_path:
            raise ValueError(
                "steer_transfer mode requires mapper_path and steer_vec_path in config."
            )
        ...
        ref_act = (
            th.load(ref_act_path, map_location="cpu", weights_only=False).float()
            if ref_act_path
            else None
        )
    ...
    # ── Mode: train — train mapper + save + run stitch inference ─────────────
        ...
        # 5. Train
        mapper = MLPMapper(
            input_dim=cfg.input_dim, output_dim=cfg.output_dim, hidden_dim=cfg.hidden_dim
        )
        ...
        if pos_prompts and neg_prompts:
            print(
                "[train] steer_pos/neg_prompts found — running steer_contrast with trained mapper..."
            )
